...
from tahlia.bot.util import (_download_file, _get_image_from_embed_url, get_image_from_message)
from tahlia.lights.scene import SceneManager
from tahlia.stream_deck.integration import (AudioPlayPlaylistKey, SwitchSceneKey, WindowImageKey)
...
class AudioManagerCog(commands.Cog):
    ...
    async def bind(self, ctx: commands.Context, url: str, label: str = ''):
        if (context := self.client.retrive_context(url)) is None or 'uri' not in context:
        ...
        with update_layout_file('music.json') as layout:
            layout.append({'type': 'playlist', 'label': label, 'image': image_path.name, 'uri': context['uri']})
        image = load_image(self.page.deck, str(image_path))
        self.page.add_key(AudioPlayPlaylistKey(image, self.client, label, context['uri']))
