...
if __name__ == "__main__":
    ...
    with use_stream_deck() as (deck, run):
        ...
        tester = TabHelperKey(
            deck,
            PredefinedKeys.PREV_PAGE,
            lambda: test_page.add_key(StaticKey(last_image, "yoo")),
        )
        test_page.add_key(tester)
        show_test_page = PageChangingKey(
            get_key_image(deck, PredefinedKeys.NEXT_PAGE), test_page, pm
        )
